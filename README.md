...
| 功能 | 正则模式 | AI 模式 |
|------|---------|---------|
| 识别常见字段（姓名/手机/学历等） | ✅ 关键词匹配 | ✅ 智能理解 |
| 填写自定义/陌生字段 | ⚠ 黄色标记未识别 | ✅ AI 自动推断 |
| 无需 API Key | ✅ | ❌ 需要配置 |
| JD 感知定制填写 | ❌ | ✅ 结合 JD 调优内容 |
| 自动补全简历缺失字段 | ❌ | ✅ 自动添加到模板 |
| BOSS 直聘 React 下拉框 | ✅ Fiber 回调 | ✅ |
| 速度 | 快 | 依网络 |
...
1. 点击插件图标 → 「⚙ 管理简历」
2. 填写个人信息，点击「💾 保存简历」
3. 如需 AI 模式，在右侧「🤖 AI 填写设置」输入 API Key 并保存
...
> **PDF 说明**：支持 Word/WPS 导出的文字型 PDF。扫描件（图片型）不支持，请先 OCR 转为 TXT。
